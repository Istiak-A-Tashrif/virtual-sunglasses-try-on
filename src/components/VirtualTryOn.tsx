...
const VirtualTryOn = () => {
  ...
  const glassesSrc = "/images/3dModels/scene.gltf";
  ...
  useEffect(() => {
    ...
    const boundingBox = new THREE.Box3().setFromObject(glassesMesh);
        const modelWidth = boundingBox.getSize(new THREE.Vector3()).x;

        let previousScale = 1
        const scaleMultiplier = eyeDistance / modelWidth;
        if (Math.abs(scaleMultiplier - previousScale.current) > 0.05) {
          glassesMesh.scale.set(scaleMultiplier, scaleMultiplier, scaleMultiplier);
          previousScale.current = scaleMultiplier;
        }
